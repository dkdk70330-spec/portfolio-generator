...
elements.characterPreviewModalTags.innerHTML = [
  ...
  async function stripPngAncillaryChunks(file, label = "PNG 이미지") {
    ...
    chunks.forEach((chunk) => {
      sanitizedBytes.set(chunk, cursor);
      cursor += chunk.length;
    });

    return new Blob([sanitizedBytes], { type: "image/png" });
  }
  ...
  async function sanitizePng(file, label = "PNG 이미지") {
    ...
    try {
      ...
      const strippedBlob = await stripPngAncillaryChunks(blob, label);

      if (strippedBlob.size > MAX_IMAGE_FILE_BYTES) {
    ...
    async function packageCatalogIcons(items, sourceBuilder) {
      for (const item of items) {
        ...
        if (!catalogAssetPaths.has(destination)) {
          try {
            const fetchedBlob = await fetchDeployBlob(
